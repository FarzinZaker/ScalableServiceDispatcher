...
class AppController {
    ...
    def jsonList() {
        ...
        def list = App.findAllByDeleted(false, parameters)
        value.total = App.countByDeleted(false)
    ...
    @Secured([RoleHelper.ROLE_ADMIN])
    def create() {
        render(template: 'form', model: [item: new App()])
    }

    @Secured([RoleHelper.ROLE_ADMIN])
    def edit() {
        render(template: 'form', model: [item: App.get(params.id)])
    }
